...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitpulsed-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitpulsed-cli', help='bitpulsed-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='bitpulsed address to which the faucet should send')
...
parser.add_argument('bitpulsed_cli_args', nargs='*', help='Arguments to pass on to bitpulsed-cli (default: -signet)')

args = parser.parse_args()

if args.bitpulsed_cli_args == []:
    args.bitpulsed_cli_args = ['-signet']


def bitpulsed_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitpulsed_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitpulsed_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitpulsed_cli(['getnewaddress', 'faucet', 'bech32'])
