...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitpulse-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitpulse-cli', help='bitpulse-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='bitpulse address to which the faucet should send')
...
parser.add_argument('bitpulse_cli_args', nargs='*', help='Arguments to pass on to bitpulse-cli (default: -signet)')

args = parser.parse_args()

if args.bitpulse_cli_args == []:
    args.bitpulse_cli_args = ['-signet']


def bitpulse_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitpulse_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitpulse_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitpulse_cli(['getnewaddress', 'faucet', 'bech32'])
