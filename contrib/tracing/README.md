...
Tracing (USDT) support for bitpulse Core on Linux using. For more information on
USDT support in bitpulse Core see the [USDT documentation].
...
[bitpulse Core USDT documentation] on how to list available tracepoints in your
`bitpulsed` binary.

[bitpulse Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
bitpulse P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
In a different terminal, starting bitpulse Core in SigNet mode and with
re-indexing enabled.
