...
""" Example logging bitpulsed Core mempool events using the mempool:added,
...
def main(bitpulsed_path):
    bitpulsed_with_usdts = USDT(path=str(bitpulsed_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bitpulsed_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bitpulsed_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bitpulsed_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bitpulsed_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bitpulsed_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bitpulsed")
        exit(1)
