# bash programmable completion for bitpulsed-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $bitpulsed-cli for RPC
_bitpulsed_rpc() {
...
_bitpulsed_cli() {
    ...
    # save and use original argument to invoke bitpulsed-cli for -help, help and RPC
    # as bitpulsed-cli might not be in $PATH
    bitpulsed_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($bitpulsed_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_bitpulsed_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _bitpulsed_cli bitpulsed-cli
