...
if [ "$CI_OS_NAME" == "macos" ]; then
  echo > "${HOME}/Library/Application Support/bitpulsed"
else
  echo > "${HOME}/.bitpulsed"
fi
...
bitpulsed_CONFIG_ALL="--disable-dependency-tracking"
if [ -z "$NO_DEPENDS" ]; then
  bitpulsed_CONFIG_ALL="${bitpulsed_CONFIG_ALL} CONFIG_SITE=$DEPENDS_DIR/$HOST/share/config.site"
fi
if [ -z "$NO_WERROR" ]; then
  bitpulsed_CONFIG_ALL="${bitpulsed_CONFIG_ALL} --enable-werror"
fi
...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  make distclean || true
  ./autogen.sh
  bash -c "./configure $bitpulsed_CONFIG_ALL $bitpulsed_CONFIG" || ( (cat config.log) && false)
  make "${MAKEJOBS}" && cd src/qt && ANDROID_HOME=${ANDROID_HOME} ANDROID_NDK_HOME=${ANDROID_NDK_HOME} make apk
  bash -c "${PRINT_CCACHE_STATISTICS}"
  exit 0
fi

bitpulsed_CONFIG_ALL="${bitpulsed_CONFIG_ALL} --enable-external-signer --prefix=$BASE_OUTDIR"
...
bash -c "${BASE_ROOT_DIR}/configure --cache-file=config.cache $bitpulsed_CONFIG_ALL $bitpulsed_CONFIG" || ( (cat config.log) && false)

make distdir VERSION="$HOST"

cd "${BASE_BUILD_DIR}/bitpulsed-$HOST"

bash -c "./configure --cache-file=../config.cache $bitpulsed_CONFIG_ALL $bitpulsed_CONFIG" || ( (cat config.log) && false)
...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  DIR_UNIT_TEST_DATA="${DIR_UNIT_TEST_DATA}" LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${BASE_OUTDIR}"/bin/test_bitpulsed --catch_system_errors=no -l test_suite
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/bitpulsed-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target bitpulsed-tidy-tests "$MAKEJOBS"

  set -eo pipefail
  cd "${BASE_BUILD_DIR}/bitpulsed-$HOST/src/"
  ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libbitpulsed-tidy.so" "${MAKEJOBS}" ) | grep -C5 "error"
  ...
  python3 "/include-what-you-use/iwyu_tool.py" \
           -p . "${MAKEJOBS}" \
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_BUILD_DIR}/bitpulsed-$HOST/contrib/devtools/iwyu/bitpulsed.core.imp" \
