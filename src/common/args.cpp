...
const char * const bitpulse_CONF_FILENAME = "bitpulse.conf";
const char * const bitpulse_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitpulse-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", bitpulse_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bitpulse
    // macOS: ~/Library/Application Support/Bitpulse
    // Unix-like: ~/.bitpulse
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitpulse";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bitpulse";
#else
    // Unix-like
    return pathRet / ".bitpulse";
#endif
#endif
}
