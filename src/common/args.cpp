...
const char * const bitpulsed_CONF_FILENAME = "bitpulsed.conf";
const char * const bitpulsed_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitpulsed-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", bitpulsed_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\bitpulsed
    // macOS: ~/Library/Application Support/bitpulsed
    // Unix-like: ~/.bitpulsed
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bitpulsed";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/bitpulsed";
#else
    // Unix-like
    return pathRet / ".bitpulsed";
#endif
#endif
}
