...
bool CWallet::LoadToWallet(const uint256& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    // If wallet doesn't have a chain (e.g when using bitpulsed-wallet tool),
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        // https://github.com/bitcoin-core/bitpulsed-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
...
{
    LOCK(cs_wallet);
    WalletLogPrintf("CommitTransaction:\n%s", tx->ToString()); // NOLINT(bitpulsed-unterminated-logprintf)
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart bitpulsed with -walletcrosschain to override.");
                return false;
            }
        }
    }
