...
QString formatBitpulseURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bitpulse:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openBitpulseConf()
{
    ...
    /* Open bitpulse.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "bitpulse.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bitpulse-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a bitpulse.desktop file to the autostart directory:
