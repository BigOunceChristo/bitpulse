...
const int bitpulse_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString bitpulse_IPC_PREFIX("bitpulse:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(bitpulse_IPC_PREFIX, Qt::CaseInsensitive)) // bitpulse: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(bitpulse_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(bitpulse_IPC_CONNECT_TIMEOUT);
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click bitpulse: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start bitpulse: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling bitpulse: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bitpulse://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bitpulse://' is not a valid URI. Use 'bitpulse:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(bitpulse_IPC_PREFIX, Qt::CaseInsensitive)) // bitpulse: URI
    {
