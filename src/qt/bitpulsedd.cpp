...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<bitpulseUnit>("bitpulseUnit");
#else
    qRegisterMetaType<bitpulseUnit>("bitpulseUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitpulse_de.qm (shortcut "de" needs to be defined in bitpulse.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitpulse_de_DE.qm (shortcut "de_DE" needs to be defined in bitpulse.qrc)
...
static const char* qt_argv = "bitpulse-qt";

bitpulseApplication::bitpulseApplication()
...
void bitpulseApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the bitpulseApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", bitpulseGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void bitpulseApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool bitpulseApplication::createOptionsModel(bool resetSettings)
{
...
void bitpulseApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new bitpulseGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &bitpulseGUI::quitRequested, this, &bitpulseApplication::requestShutdown);
...
void bitpulseApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void bitpulseApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool bitpulseApplication::baseInitialize()
{
    return node().baseInitialize();
}

void bitpulseApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &bitpulseApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &bitpulseApplication::handleRunawayException);
    connect(this, &bitpulseApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &bitpulseApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void bitpulseApplication::parameterSetup()
{
...
void bitpulseApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void bitpulseApplication::requestInitialize()
{
...
void bitpulseApplication::requestShutdown()
{
...
void bitpulseApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bitpulse: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &bitpulseGUI::handlePaymentRequest);
            connect(window, &bitpulseGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void bitpulseApplication::handleRunawayException(const QString &message)
{
...
void bitpulseApplication::handleNonFatalException(const QString& message)
{
...
WId bitpulseApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool bitpulseApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", bitpulseGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitpulse);
    Q_INIT_RESOURCE(bitpulse_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(bitpulse_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitpulse-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitpulse-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitpulse.conf, determine network, switch to network specific
    ...
    // bitpulse: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
