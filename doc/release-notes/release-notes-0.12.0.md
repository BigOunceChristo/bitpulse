bitpulse Core version 0.12.0 is now available from:

  <https://bitpulse.org/bin/bitpulse-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of bitpulse Core or other software:
...
pre-0.12 versions of bitpulse Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start bitpulse Core version 0.11 or
earlier.
...
ECDSA signatures inside bitpulse transactions now use validation using
...
Previous versions of bitpulse Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of bitpulse Core was to raise the default minimum
relay fee.

bitpulse Core 0.12 will have a strict maximum size on the mempool. The
...
bitpulse Core 0.12 also introduces new default policy limits on the length and
...
bitpulse Core 0.12 nodes. bitpulse Core will only allow replacement of
...
Note that the wallet in bitpulse Core 0.12 does not yet have support for
...
bitpulse Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. bitpulse Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In bitpulse Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in bitpulse Core 0.13, and it is at this time undecided whether the
...
bitpulse Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
bitpulse Core automatically creates a hidden service to listen on, without
manual configuration. bitpulse Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if bitpulse Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals bitpulse
Core to use floating fees. By default, bitpulse Core will use floating
fees.
...
At all times, bitpulse Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTC.
Furthermore, bitpulse Core will never create transactions paying less than
...
`bitpulse.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitpulserpc>
        ...
        # get the <hash> from the shell with: base64 <<< bitpulserpc:<password>
    </Location>
...
- #6214 `87406aa` [OSX] revert renaming of bitpulse-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new bitpulse-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include bitpulse-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix bitpulse-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of bitpulse-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override bitpulseD and bitpulseCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [bitpulse-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add bitpulseD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libbitpulseconsensus (Braydon Fuller)
