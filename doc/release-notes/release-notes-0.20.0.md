...
bitpulsed Core version 0.20.0 is now available from:

  <https://bitpulsedcore.org/bin/bitpulsed-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  bitpulsed
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use bitpulsed Core on
unsupported systems.

From bitpulsed Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, bitpulsed Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `bitpulsed_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from bitpulsed Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since bitpulsed Core version 0.18.0.
...
- Testing or debugging of implementations of the bitpulsed P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of bitpulsed Core. bitpulsed Core logs debug messages
...
- OpenSSL is no longer used by bitpulsed Core.  (#17265)

- BIP70 support has been fully removed from bitpulsed Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `bitpulsed help` for more information.  This option is experimental and
...
- All custom settings configured when bitpulsed Core starts are now
...
- The "Start bitpulsed Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitpulsedcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple bitpulsed Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed bitpulsed Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of bitpulsed Core should support a GUI option
...
- bitpulsed Core's automatically-generated source code documentation is
  now available at https://doxygen.bitpulsedcore.org.  (#17596)
...
- The `bitpulsed-cli` utility used with the `-getinfo` parameter now
...
- The `bitpulsed-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitpulsedstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitpulsed-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBbitpulsed_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bitpulsed` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBbitpulsed_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bitpulsed-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitpulsed schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitpulsed.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitpulsed` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitpulsed-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitpulsed-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about bitpulsed Core files (hebasto)
...
- #17119 Fix broken bitpulsed-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitpulsed-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitpulsedcore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitpulsed-wallet (hel-o)
...
- #16115 On bitpulsed startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitpulsed-cli -getinfo (malevolent)
