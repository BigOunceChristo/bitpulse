...
bitpulse Core version 0.20.0 is now available from:

  <https://bitpulsecore.org/bin/bitpulse-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  bitpulse
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use bitpulse Core on
unsupported systems.

From bitpulse Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, bitpulse Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `bitpulse_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from bitpulse Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since bitpulse Core version 0.18.0.
...
- Testing or debugging of implementations of the bitpulse P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of bitpulse Core. bitpulse Core logs debug messages
...
- OpenSSL is no longer used by bitpulse Core.  (#17265)

- BIP70 support has been fully removed from bitpulse Core. The
...
- All custom settings configured when bitpulse Core starts are now
...
- The "Start bitpulse Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitpulsecore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple bitpulse Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed bitpulse Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of bitpulse Core should support a GUI option
...
- bitpulse Core's automatically-generated source code documentation is
  now available at https://doxygen.bitpulsecore.org.  (#17596)
...
- The `bitpulse-cli` utility used with the `-getinfo` parameter now
...
- The `bitpulse-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitpulsestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitpulse-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBbitpulse_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bitpulse` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBbitpulse_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bitpulse-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitpulse schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitpulse.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitpulse` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitpulse-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitpulse-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about bitpulse Core files (hebasto)
...
- #17119 Fix broken bitpulse-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitpulse-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitpulsecore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitpulse-wallet (hel-o)
...
- #17650 util: Remove unwanted fields from bitpulse-cli -getinfo (malevolent)
