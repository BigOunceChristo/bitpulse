bitpulsed Core version 0.18.0 is now available from:

  <https://bitpulsedcore.org/bin/bitpulsed-core-0.18.0/>
...
`/Applications/bitpulsed-Qt` (on Mac) or `bitpulsed`/`bitpulsed-qt` (on
Linux).
...
recommended to use bitpulsed Core on unsupported systems.
...
10.10. Additionally, bitpulsed Core does not yet change appearance when
...
`contrib/init/bitpulsed.service`, it has been changed to use
`/var/lib/bitpulsed` as the data directory instead of
`~bitpulsed/.bitpulsed`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitpulsed` will
exist has enough space (check using `df -h /var/lib/bitpulsed`), and
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitpulsedcore.org/en/contact/) the bitpulsed Core
...
The systemd init file (`contrib/init/bitpulsed.service`) has been changed
to use `/var/lib/bitpulsed` as the data directory instead of
`~bitpulsed/.bitpulsed`. This change makes bitpulsed Core more consistent with
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/bitpulsed-conf.md)
  about the `bitpulsed.conf` file describes how to use it to configure
  bitpulsed Core.

- A new document introduces bitpulsed Core's BIP174 [Partially-Signed
  bitpulsed Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent bitpulsed-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  bitpulsed Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when bitpulsed Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of bitpulsed Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitpulsed with the `-deprecatedrpc=generate`
...
- A new `bitpulsed-wallet` tool is now distributed alongside bitpulsed
...
This section describes planned changes to bitpulsed Core that may affect
other bitpulsed software and services.

- Since version 0.16.0, bitpulsed Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with bitpulsed Core 0.20 (expected about
  a year after 0.18), bitpulsed Core will default to native segwit
  ...
  to bech32 addresses, and if the bitpulsed Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in bitpulsed Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any bitpulsed Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, bitpulsed Core
...
- A sub-project of bitpulsed Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with bitpulsed Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to bitpulsed Core's own implementation, although entropy
  gathered by bitpulsed Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves bitpulsed Core a
...
- On macOS, bitpulsed Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitpulsed_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitpulsed_server.a` (jnewbery)
...
- #13926 [Tools] bitpulsed-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitpulsed (MarcoFalke)
...
- #14521 qt, docs: Fix `bitpulsed-qt -version` output formatting (hebasto)
...
- #11625 Add bitpulsedApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitpulsed_server` (laanwj)
...
- #15154 Configure: bitpulsed-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitpulsed-wallet (Sjors)
...
- #11526 Visual Studio build configuration for bitpulsed Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitpulsed_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitpulsed-util-test.py (isghe)
...
- #12775 Integration of property based testing into bitpulsed Core (Christewart)
...
- #12246 Bugfix: Only run bitpulsed-tx tests when bitpulsed-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitpulsed` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitpulsed` (MarcoFalke)
...
- #14186 bitpulsed-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitpulsed.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitpulsed.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitpulsed-conf.md (hebasto)
...
- #15354 Add missing `bitpulsed-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitpulsedconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitpulsed folder on OSX (JimmyMow)
