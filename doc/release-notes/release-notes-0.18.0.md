bitpulse Core version 0.18.0 is now available from:

  <https://bitpulsecore.org/bin/bitpulse-core-0.18.0/>
...
`/Applications/bitpulse-Qt` (on Mac) or `bitpulsed`/`bitpulse-qt` (on
Linux).
...
recommended to use bitpulse Core on unsupported systems.
...
10.10. Additionally, bitpulse Core does not yet change appearance when
...
`~bitpulse/.bitpulse`. When switching over to the new configuration file,
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitpulsecore.org/en/contact/) the bitpulse Core
...
`~bitpulse/.bitpulse`. This change makes bitpulse Core more consistent with
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/bitpulse-conf.md)
  about the `bitpulse.conf` file describes how to use it to configure
  bitpulse Core.

- A new document introduces bitpulse Core's BIP174 [Partially-Signed
  bitpulse Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent bitpulse-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  bitpulse Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when bitpulse Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of bitpulse Core, subsequently importing
...
- A new `bitpulse-wallet` tool is now distributed alongside bitpulse
...
This section describes planned changes to bitpulse Core that may affect
other bitpulse software and services.

- Since version 0.16.0, bitpulse Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with bitpulse Core 0.20 (expected about
  a year after 0.18), bitpulse Core will default to native segwit
  ...
  to bech32 addresses, and if the bitpulse Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in bitpulse Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any bitpulse Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, bitpulse Core
...
- A sub-project of bitpulse Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with bitpulse Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to bitpulse Core's own implementation, although entropy
  gathered by bitpulse Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves bitpulse Core a
...
- On macOS, bitpulse Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitpulse_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitpulse_server.a` (jnewbery)
...
- #13926 [Tools] bitpulse-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14521 qt, docs: Fix `bitpulse-qt -version` output formatting (hebasto)
...
- #11625 Add bitpulseApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitpulse_server` (laanwj)
...
- #15154 Configure: bitpulse-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitpulse-wallet (Sjors)
...
- #11526 Visual Studio build configuration for bitpulse Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitpulse_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitpulse-util-test.py (isghe)
...
- #12775 Integration of property based testing into bitpulse Core (Christewart)
...
- #12246 Bugfix: Only run bitpulse-tx tests when bitpulse-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitpulse` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitpulse` (MarcoFalke)
...
- #14186 bitpulse-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitpulse.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitpulse.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitpulse-conf.md (hebasto)
...
- #15354 Add missing `bitpulse-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitpulseconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitpulse folder on OSX (JimmyMow)
