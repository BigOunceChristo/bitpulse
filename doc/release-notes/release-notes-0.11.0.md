bitpulse Core version 0.11.0 is now available from:

  <https://bitpulse.org/bin/bitpulse-core-0.11.0/>
...
For example, add the following to `bitpulse.conf`:
...
related to the blockchain in the bitpulse system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows bitpulse Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that bitpulse Core will 
...
For an example script see [bitpulse-submittx](https://github.com/laanwj/bitpulse-submittx).
...
- #5334 `cf87536` libbitpulseconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitpulse-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only bitpulseAmountField
- #5753 `0f44672` Add bitpulse logo to about screen
...
- #5809 `46bfbe7` Add bitpulse-cli man page
...
- #6116 `30dc3c1` [OSX] rename bitpulse-Qt.app to bitpulse-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitpulse-tx)
