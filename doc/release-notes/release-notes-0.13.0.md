bitpulse Core version 0.13.0 is now available from:

  <https://bitpulse.org/bin/bitpulse-core-0.13.0/>
...
- Adding `dbcache=100` in bitpulse.conf
...
bitpulse-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bitpulse-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The bitpulse Core code base has
...
HD wallets are incompatible with older versions of bitpulse Core.
...
proposed parameters, a future release of bitpulse Core is expected that
...
The two phases are distinct in the bitpulse-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for bitpulse-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to bitpulse-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add bitpulse address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: bitpulse-qt crash (fsb4000)
...
- #8207 `f7a403b` Add a link to the bitpulse-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [bitpulse-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7975 `fc23fee` Update bitpulse-core GitHub links (MarcoFalke)
