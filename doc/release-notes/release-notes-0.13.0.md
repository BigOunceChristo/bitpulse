bitpulsed Core version 0.13.0 is now available from:

  <https://bitpulsed.org/bin/bitpulsed-core-0.13.0/>
...
- Adding `dbcache=100` in bitpulsed.conf
...
bitpulsed-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bitpulsed-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The bitpulsed Core code base has
...
HD wallets are incompatible with older versions of bitpulsed Core.
...
proposed parameters, a future release of bitpulsed Core is expected that
...
The two phases are distinct in the bitpulsed-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for bitpulsed-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to bitpulsed-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add bitpulsed address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: bitpulsed-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from bitpulsed (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the bitpulsed-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for bitpulsed procs (MarcoFalke)
...
- #8164 `0f24eaf` [bitpulsed-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of bitpulsed startup (laanwj)
...
- #7975 `fc23fee` Update bitpulsed-core GitHub links (MarcoFalke)
