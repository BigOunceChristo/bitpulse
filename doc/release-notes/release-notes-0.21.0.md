...
bitpulsed Core version 0.21.0 is now available from:

  <https://bitpulsedcore.org/bin/bitpulsed-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by bitpulsed Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. bitpulsed Core will no longer
...
- A new `bitpulsed-cli -netinfo` command provides a network peer connections
...
- A new `bitpulsed-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `bitpulsed-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `bitpulsed-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when bitpulsed Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `bitpulsed-wallet` tool which performs the salvage
...
bitpulsed Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitpulsed.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
bitpulsed Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `bitpulsed-cli createwallet` or `bitpulsed-wallet
...
Users who already have a bitpulsed Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of bitpulsed Core.
...
The change to using descriptors changes the default derivation paths used by bitpulsed Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time bitpulsed Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `bitpulsed.conf`
...
- #20198 Show name, format and if uses descriptors in bitpulsed-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add bitpulsed-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in bitpulsed-cli (laanwj)
...
### Build system
- #18504 Drop bitpulsed-tx and bitpulsed-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in bitpulsed_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for bitpulsed-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz bitpulsed Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrebitpulsed)
...
- #18817 Document differences in bitpulsed and bitpulsed-qt locale handling (practicalswift)
