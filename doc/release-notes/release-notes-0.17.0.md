bitpulse Core version 0.17.0 is now available from:

  <https://bitpulsecore.org/bin/bitpulse-core-0.17.0/>
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `bitpulse.conf` file, not inside included files or from
  ...
  as bitpulse.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the bitpulse.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=bitpulse
    test.uacomment=bitpulse-testnet
...
BIP 174 Partially Signed bitpulse Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for bitpulse transactions that are not fully signed
...
Overall, the construction of a fully signed bitpulse transaction goes through the
following steps:
...
- An **Extractor** produces a valid bitpulse transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When bitpulse is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If bitpulse is started
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI bitpulse:// (krab)
...
- #13528 `b877c39` Move bitpulseGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libbitpulse-util to libbitpulse-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for bitpulse-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling bitpulse-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use bitpulseTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to bitpulse.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use bitpulseTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use bitpulseTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use bitpulseTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for bitpulseTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_bitpulse` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.bitpulse is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_bitpulse` once (MarcoFalke)
...
- #12652 `55f490a` bitpulse-cli: Provide a better error message when bitpulsed is not running (practicalswift)
...
- #13374 `56f6936` utils and libraries: checking for bitpulse address in translations (kaplanmaxe)
...
- #13603 `453ae5e` bitpulse-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix bitpulse-cli --version (Empact)
...
- #13346 `60f0358` update bitpulse-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust bitpulsecore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed bitpulse-cli -help output for help2man (hebasto)
