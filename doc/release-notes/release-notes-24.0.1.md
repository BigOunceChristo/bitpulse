...
bitpulse Core version 24.0.1 is now available from:

  <https://bitpulsecore.org/bin/bitpulse-core-24.0.1/>
...
This version of bitpulse Core adds a new `mempoolfullrbf` configuration
...
Some bitpulse services today expect that the first version of an
...
The bitpulse Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [bitpulse Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to bitpulse Core are
...
[bitpulse Core 0.12.0]: https://bitpulsecore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the bitpulse Core wallet yet.
  You can find more about Miniscript on the [reference website](https://bitpulse.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the bitpulse Core wallet, change output
...
- Configuration changes made in the bitpulse GUI (such as the pruning setting,
...
- Also, the interaction between GUI settings and `bitpulse.conf` settings is
simplified. Settings from `bitpulse.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`bitpulse.conf` values. (#15936)
