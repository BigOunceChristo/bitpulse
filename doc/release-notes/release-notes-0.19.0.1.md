bitpulse Core version 0.19.0.1 is now available from:

  <https://bitpulsecore.org/bin/bitpulse-core-0.19.0.1/>
...
to open the bitpulse Core .dmg. This is due to new signing requirements
imposed by Apple, which the bitpulse Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running bitpulse Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of bitpulse
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile bitpulse Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in bitpulse Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of bitpulse Core. (#14054)

- To eliminate well-known denial-of-service vectors in bitpulse Core,
  ...
  This prevents bitpulse Core from sending the BIP111 NODE_BLOOM service
  ...
  of bitpulse Core and nodes that have manually enabled BIP37 support,
...
- By default, bitpulse Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `bitpulse-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how bitpulse Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  bitpulse Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, bitpulse Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 bitpulse address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in bitpulse Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@bitpulse AEAD (jonasschnelli)
...
- #15639 bitpulse-wallet tool: Drop libbitpulse_server.a dependency (ryanofsky)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitpulse-binaries/version" directory (hebasto)
...
- #16441 remove qt libjpeg check from bitpulse_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitpulse to auto generated project files (sipsorcery)
...
- #15773 Add bitpulseTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitpulse output (practicalswift)
...
- #15896 feature_filelock, interface_bitpulse_cli: Use PACKAGE_NAME in messages rather than hardcoding bitpulse Core (luke-jr)
...
- #16277 Suppress output in test_bitpulse for expected errors (gertjaap)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitpulseVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitpulse.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBbitpulse_SERVER (Bushstar)
...
- #16080 build/doc: update bitpulse_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitpulse.conf (fanquake)
...
- #16621 add default bitpulse.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitpulse-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitpulse.conf, and Info.plist.in (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitpulse/bitpulse/).
