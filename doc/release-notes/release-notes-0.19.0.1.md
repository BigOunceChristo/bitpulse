bitpulsed Core version 0.19.0.1 is now available from:

  <https://bitpulsedcore.org/bin/bitpulsed-core-0.19.0.1/>
...
to open the bitpulsed Core .dmg. This is due to new signing requirements
imposed by Apple, which the bitpulsed Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running bitpulsed Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of bitpulsed
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitpulsed -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile bitpulsed Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in bitpulsed Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of bitpulsed Core. (#14054)

- To eliminate well-known denial-of-service vectors in bitpulsed Core,
  ...
  This prevents bitpulsed Core from sending the BIP111 NODE_BLOOM service
  ...
  of bitpulsed Core and nodes that have manually enabled BIP37 support,
...
- By default, bitpulsed Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `bitpulsed-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how bitpulsed Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  bitpulsed Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, bitpulsed Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 bitpulsed address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in bitpulsed Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@bitpulsed AEAD (jonasschnelli)
...
- #15639 bitpulsed-wallet tool: Drop libbitpulsed_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in bitpulsed/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitpulsed-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bitpulsed build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bitpulsed_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitpulsed to auto generated project files (sipsorcery)
...
- #15773 Add bitpulsedTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitpulsed output (practicalswift)
...
- #15896 feature_filelock, interface_bitpulsed_cli: Use PACKAGE_NAME in messages rather than hardcoding bitpulsed Core (luke-jr)
...
- #16277 Suppress output in test_bitpulsed for expected errors (gertjaap)
...
- #16489 log: harmonize bitpulsed logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitpulsedVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitpulsed.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBbitpulsed_SERVER (Bushstar)
...
- #16080 build/doc: update bitpulsed_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitpulsed.conf (fanquake)
...
- #16621 add default bitpulsed.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitpulsed-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitpulsed.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bitpulsed.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitpulsed/bitpulsed/).
