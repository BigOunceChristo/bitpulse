...
The bitpulse Core daemon can be configured to act as a trusted "border
router", implementing the bitpulse wire protocol and relay, making
...
The ZeroMQ feature in bitpulse Core requires the ZeroMQ API >= 4.0.0
...
terminator). These options can also be provided in bitpulse.conf.
