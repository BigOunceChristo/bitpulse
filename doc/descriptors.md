# Support for Output Descriptors in bitpulsed Core

Since bitpulsed Core v0.17, there is support for Output Descriptors. This is a
...
on bitpulsed's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a bitpulsed Core descriptor wallet to serve as your
...
existing bitpulsed Core wallets, a convenience function `combo` is
...
All RPCs in bitpulsed Core will include the checksum in their output. Only
