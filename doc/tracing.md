# User-space, Statically Defined Tracing (USDT) for bitpulsed Core

bitpulsed Core includes statically defined tracepoints to allow for more
...
**Note**: bitpulsed Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to bitpulsed Core
...
Multiple tools can list the available tracepoints in a `bitpulsed` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in bitpulsed Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bitpulsed
stap net        outbound_message 0x0000000000107c05 /src/bitpulsed
stap validation block_connected  0x00000000002fb10c /src/bitpulsed
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in bitpulsed Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitpulsed | grep NT_STAPSDT -A 4 -B 2
