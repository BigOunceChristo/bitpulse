# User-space, Statically Defined Tracing (USDT) for bitpulse Core

bitpulse Core includes statically defined tracepoints to allow for more
...
**Note**: bitpulse Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to bitpulse Core
...
To list probes in bitpulse Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in bitpulse Core.
