# CJDNS support in bitpulse Core

It is possible to run bitpulse Core over CJDNS, an encrypted IPv6 network that
...
done outside of bitpulse Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the bitpulse network and individual nodes.
...
before it will work with your bitpulse Core node. This is described in steps
...
## Run bitpulse Core with CJDNS

Once you are connected to the CJDNS network, the following bitpulse Core
...
When enabled, this option tells bitpulse Core that it is running in an
...
IPv6 local network. This helps bitpulse Core perform better address management:
...
CJDNS support was added to bitpulse Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitpulse-cli -addrinfo` to see the
...
## CJDNS-related information in bitpulse Core

There are several ways to see your CJDNS address in bitpulse Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitpulse-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitpulse-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitpulse-cli help getnodeaddresses` for details.
