# CJDNS support in bitpulsed Core

It is possible to run bitpulsed Core over CJDNS, an encrypted IPv6 network that
...
done outside of bitpulsed Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the bitpulsed network and individual nodes.
...
before it will work with your bitpulsed Core node. This is described in steps
...
## Run bitpulsed Core with CJDNS

Once you are connected to the CJDNS network, the following bitpulsed Core
...
When enabled, this option tells bitpulsed Core that it is running in an
...
IPv6 local network. This helps bitpulsed Core perform better address management:
...
CJDNS support was added to bitpulsed Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitpulsed-cli -addrinfo` to see the
...
## CJDNS-related information in bitpulsed Core

There are several ways to see your CJDNS address in bitpulsed Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitpulsed-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitpulsed-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitpulsed-cli help getnodeaddresses` for details.
