...
- [Development guidelines](#development-guidelines)
    - [General bitpulsed Core](#general-bitpulsed-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
bitpulsed Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while bitpulsed is running with the `logging`
RPC.  For instance, launching bitpulsed with `-debug` or `-debug=1` will turn on
...
with "play bitpulseds" on a test network.
...
bitpulsed Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run bitpulsed.

You can then use the `-debug=lock` configuration option at bitpulsed startup or
`bitpulsed-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `bitpulsed-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bitpulsed
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitpulsed --log_level=test_suite
$ valgrind -v --leak-check=full src/bitpulsed -printtoconsole
...
# A coverage report will now be accessible at `./test_bitpulsed.coverage/index.html`,
...
To profile a running bitpulsed process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bitpulsed` -- sleep 60
```
...
bitpulsed Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`bitpulsed`)](https://doxygen.bitpulsedcore.org/bitpulsed_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `bitpulsed.cpp`. Responsible for starting up and
  shutting down the application.

- [Init load (`b-initload`)](https://doxygen.bitpulsedcore.org/namespacenode.html#ab4305679079866f0f420f7dbf278381d)
...
- [CCheckQueue::Loop (`b-scriptch.x`)](https://doxygen.bitpulsedcore.org/class_c_check_queue.html#a6e7fa51d3a25e7cb65446d4b50e6a987)
  : Parallel script validation threads for transactions in blocks.

- [ThreadHTTP (`b-http`)](https://doxygen.bitpulsedcore.org/httpserver_8cpp.html#abb9f6ea8819672bd9a62d3695070709c)
  : Libevent thread to listen for RPC and REST connections.

- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.bitpulsedcore.org/httpserver_8cpp.html#aa6a7bc27265043bc0193220c5ae3a55f)
  : Threads to service RPC and REST requests.

- [Indexer threads (`b-txindex`, etc)](https://doxygen.bitpulsedcore.org/class_base_index.html#a96a7407421fbf877509248bbe64f8d87)
  : One thread per indexer.

- [SchedulerThread (`b-scheduler`)](https://doxygen.bitpulsedcore.org/class_c_scheduler.html#a14d2800815da93577858ea078aed1fba)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.bitpulsedcore.org/torcontrol_8cpp.html#a52a3efff23634500bb42c6474f306091)
  : Libevent thread for tor connections.

- Net threads:

  - [ThreadMessageHandler (`b-msghand`)](https://doxygen.bitpulsedcore.org/class_c_connman.html#aacdbb7148575a31bb33bc345e2bf22a9)
  ...
  - [ThreadDNSAddressSeed (`b-dnsseed`)](https://doxygen.bitpulsedcore.org/class_c_connman.html#aa7c6970ed98a4a7bafbc071d24897d13)
    : Loads addresses of peers from the DNS.

  - ThreadMapPort (`b-mapport`)
    : Universal plug-and-play startup/shutdown.

  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitpulsedcore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 8333.

  - [ThreadOpenAddedConnections (`b-addcon`)](https://doxygen.bitpulsedcore.org/class_c_connman.html#a0b787caf95e52a346a2b31a580d60a62)
    : Opens network connections to added nodes.

  - [ThreadOpenConnections (`b-opencon`)](https://doxygen.bitpulsedcore.org/class_c_connman.html#a55e9feafc3bab78e5c9d408c207faa45)
    : Initiates new connections to peers.

  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.bitpulsedcore.org/class_c_connman.html#a57787b4f9ac847d24065fbb0dd6e70f8)
...
However, in open source software such as bitpulsed Core, where everyone uses
...
pay attention to for reviewers of bitpulsed Core code.

General bitpulsed Core
----------------------
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: bitpulsed Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `bitpulsed_FOO_BAR_H`, e.g.

```c++
#ifndef bitpulsed_FOO_BAR_H
#define bitpulsed_FOO_BAR_H
...
#endif // bitpulsed_FOO_BAR_H
```
...
Some of these are maintained by active developers of bitpulsed Core, in which case
...
a bitpulsed Core subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with bitpulsed's `select()` loop, because
...
compatibility between nodes. This happened in bitpulsed 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to bitpulsed's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitpulsed-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitpulsed-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of bitpulsed Core, the wallet was always
