...
* Update bitpulsed.conf and commit, see [gen-bitpulsed-conf.sh](https://github.com/bitcoin/bitcoin/blob/master/contrib/devtools/README.md#gen-bitpulsed-confsh).
...
- Translations on Transifex:
    ...
    - Create [a new resource](https://www.transifex.com/bitpulsed/bitpulsed/content/) named after the major version with the slug `qt-translation-<RRR>x`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/bitpulsed_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://www.transifex.com/bitpulsed/communication/) as a template.
    - Change the auto-update URL for the resource to `master`, e.g. `https://raw.githubusercontent.com/bitpulsed/bitpulsed/master/src/qt/locale/bitpulsed_en.xlf`. (Do this only after the previous steps, to prevent an auto-update from interfering.)

#### After branch-off (on the major release branch)

- Update the versions.
- Create the draft, named "*version* Release Notes Draft", as a [collaborative wiki](https://github.com/bitcoin-core/bitpulsed-devwiki/wiki/_new).
...
To tag the version (or release candidate) in git, use the `make-tag.py` script from [bitpulsed-maintainer-tools](https://github.com/bitcoin-core/bitpulsed-maintainer-tools). From the root of the repository run:

    ../bitpulsed-maintainer-tools/make-tag.py v(new version, e.g. 25.0)
...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/bitcoin-core/bitpulsed-detached-sigs.git
    git clone https://github.com/bitcoin/bitcoin.git

### Write the release notes

Open a draft of the release notes for collaborative editing at https://github.com/bitcoin-core/bitpulsed-devwiki/wiki.
...
Checkout the bitpulsed Core version you'd like to build:
...
### macOS codesigner only: Create detached macOS signatures (assuming [signapple](https://github.com/achow101/signapple/) is installed and up to date with master branch)
    ...
    tar xf bitpulsed-osx-unsigned.tar.gz
...
### Windows codesigner only: Create detached Windows signatures
    ...
    tar xf bitpulsed-win-unsigned.tar.gz
...
However if this is done, once the release has been tagged in the bitpulsed-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ./bitpulsed-detached-sigs
...
- Detached signatures will then be committed to the [bitpulsed-detached-sigs](https://github.com/bitcoin-core/bitpulsed-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
- Upload to the bitpulsedcore.org server (`/var/www/bin/bitpulsed-core-${VERSION}/`):
    1. The contents of each `./bitpulsed/guix-build-${VERSION}/output/${HOST}/` directory, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the bitpulsedcore.org server,
       nor put them in the torrent*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@bitpulsedcore.org:/var/www/bin/bitpulsed-core-${VERSION} \;
       ```
...
- Create a torrent of the `/var/www/bin/bitpulsed-core-${VERSION}` directory such
  that at the top level there is only one file: the `bitpulsed-core-${VERSION}`
  directory containing everything else. Name the torrent
  `bitpulsed-${VERSION}.torrent` (note that there is no `-core-` in this name).
  ...
  people without access to `bitpulsedcore.org` to download the binary distribution.
  Also put it into the `optional_magnetlink:` slot in the YAML file for
  bitpulsedcore.org.

- Update other repositories and websites for new version

  - bitpulsedcore.org blog post

  - bitpulsedcore.org maintained versions update:
    [table](https://github.com/bitcoin-core/bitpulsedcore.org/commits/master/_includes/posts/maintenance-table.md)
  ...
  - bitpulsedcore.org RPC documentation update

      - See https://github.com/bitcoin-core/bitpulsedcore.org/blob/master/contrib/doc-gen/

  - Update packaging repo

      - Push the flatpak to flathub, e.g. https://github.com/flathub/org.bitpulsedcore.bitpulsed-qt/pull/2
...
- Announce the release:
  ...
  - bitpulsed-dev and bitpulsed-core-dev mailing list

  - bitpulsed Core announcements list https://bitpulsedcore.org/en/list/announcements/join/

  - bitpulsed Core Twitter https://twitter.com/bitpulsedcoreorg
