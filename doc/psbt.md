# PSBT Howto for bitpulse Core

Since bitpulse Core 0.17, an RPC interface exists for Partially Signed bitpulse
...
PSBT is an interchange format for bitpulse transactions that are not fully signed
...
#### Multisig with multiple bitpulse Core instances
...
bitpulse Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitpulse-cli` is used.
