# PSBT Howto for bitpulsed Core

Since bitpulsed Core 0.17, an RPC interface exists for Partially Signed bitpulsed
...
PSBT is an interchange format for bitpulsed transactions that are not fully signed
...
#### Multisig with multiple bitpulsed Core instances
...
bitpulsed Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitpulsed-cli` is used.
