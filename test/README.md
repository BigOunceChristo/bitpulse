This directory contains integration tests that test bitpulsed and its
...
The P2P and RPC ports used by the bitpulsed nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bitpulsed
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bitpulsed nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bitpulsed processes are running.

On linux, the test framework will warn if there is another
bitpulsed process running when the tests are started.

If there are zombie bitpulsed processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bitpulsed processes running on the system, so should not be used if any non-test
bitpulsed processes are being run.**
...
sure bitpulsed processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bitpulsed `debug.log`s will all be dumped
...
`test_framework.log` and bitpulsed `debug.log`s can be combined into a single
...
call methods that interact with the bitpulsed nodes-under-test.

If further introspection of the bitpulsed instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bitpulsed.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bitpulsed <pid>
```
