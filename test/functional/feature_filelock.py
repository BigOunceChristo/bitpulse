...
class FilelockTest(bitpulsedTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second bitpulsed instance using the same datadir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second bitpulsed using the same datadir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second bitpulsed instance shutdown
        pid_file = datadir / "bitpulsed.pid"
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second bitpulsed instance using the same wallet")
                if descriptors:
